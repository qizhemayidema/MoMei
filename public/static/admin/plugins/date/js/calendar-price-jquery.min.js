...
function($) {
    ...
    formatDate = function(t, e) { / (y + ) / i.test(e) && (e = e.replace(RegExp.$1, (t.getFullYear() + "").substr(4 - RegExp.$1.length)));
        var a = {
            "M+": t.getMonth() + 1,
            "d+": t.getDate(),
            "h+": t.getHours(),
    ...
    function isLeIE9() {
        ...
        e = null;
        return /MSIE (\d+)\./i.test(t) && (e = RegExp.$1),
        e && +e <= 9
    }
    var BC = (k = {},
    {
        ...
        $emit: function(t) {
            ...
            return k[t].fun.apply(k[t].scope, e),
            this
        }
    }),
    k;
    function createSettingWindow() {
        ...
        return t.className = "capricorncd-date-detailed-settings" + (isLeIE9() ? " ie": ""),
    ...
    function _createDays() {
        for (var t = "",
        e = 1; e <= 31; e++) t += '<i class="_checkbox">' + formatNumber(e) + "</i>";
        return t
    }
    function log() {
    ...
    fn._prevMonth = function() {
        ...
        this.month = 1 === e ? this.dateToObject(t - 1 + "/12") : this.dateToObject(t + "/" + (e - 1)),
        this.createCalendar()
    },
    fn._nextMonth = function() {
        var t = toNumber(formatDate(this.month, "yyyy")),
        e = toNumber(formatDate(this.month, "MM"));
        this.month = 12 === e ? this.dateToObject(t + 1 + "/01") : this.dateToObject(t + "/" + (e + 1)),
        this.createCalendar()
    },
    ...
    fn._getEndDate = function() {
        ...
        e && a || (e = toNumber(formatDate(TODAY, "yyyy")) + 1, a = toNumber(formatDate(TODAY, "MM"))),
        r = /^\d{4}[-\.\/]\d{1,2}[-\.\/](\d{1,2})/.test(t) ? RegExp.$1: new Date(Date.parse(new Date(e, a)) - ONE_DAY_MSEC).getDate(),
        this.dateToObject(e + "/" + a + "/" + r)
    },
    fn.dateToObject = function(t) {
        ...
        return this.ev.error && this.$emit("error", _err(2, a)),
        this.opts.error(_err(2, a)),
        !1
    },
    fn.createCalendar = function() {
        ...
        a <= formatDate(this.startDate, "yyyyMM") && (t = !1),
        ...
        r += '\t\t<div class="calendar-month-title">' + formatDate(this.month, "yyyy年MM月") + "</div>",
        ...
        r += "\t    </table>",
        r += "    </div>",
        this.opts.hideFooterButton || (r += '    <div class="calendar-foot-wrapper">', r += '        <button class="btn bg-success btn-reset">重置</button>', r += '        <button class="btn bg-primary btn-confirm">确定</button>', r += '        <button class="btn bg-white btn-cancel">取消</button>', r += "    </div>"),
        r += "</div>",
    ...
    fn._getDateData = function(t) {
        for (var e, a = 0; a < this.data.length; a++) if (t === (e = this.data[a]).date) return e;
        return null
    },
    ...
    fn.initSettingWindow = function() {
        ...
        t += '       <div class="cddsw-title">设置</div>',
        t += '       <a class="cddsw-close"><i></i></a>',
        t += "   </div>",
        t += '   <ul class="cddsw-form-wrapper clearfix">',
        t += this._createDaySetupInputGroup(),
        t += "   </ul>",
        t += '   <fieldset class="cddsw-batch-settings clearfix">',
        t += '       <legend class="bs-title"><b>批量设置</b></legend>',
        t += '       <div class="bs-content">',
        t += '           <div class="bs-lable">日期范围</div>',
        t += '           <div class="bs-options-wrapper">',
        t += '               <input class="itext" name="startDay" type="date">',
        t += '               <span class="white-space">-</span>',
        t += '               <input class="itext" name="endDay" type="date">',
        t += "           </div>",
        t += "       </div>",
        t += '       <div class="bs-content bs-week-chekbox">',
        t += '           <div class="bs-lable">设置星期</div>',
        t += '           <div class="bs-options-wrapper">',
        t += '               <i class="_checkbox" data-value="1">周一</i>',
        t += '               <i class="_checkbox" data-value="2">周二</i>',
        t += '               <i class="_checkbox" data-value="3">周三</i>',
        t += '               <i class="_checkbox" data-value="4">周四</i>',
        t += '               <i class="_checkbox" data-value="5">周五</i>',
        t += '               <i class="_checkbox" data-value="6">周六</i>',
        t += '               <i class="_checkbox" data-value="0">周日</i>',
        t += "           </div>",
        t += "       </div>",
        t += '       <div class="bs-content bs-days-select">',
        t += '           <div class="bs-lable">指定日期</div>',
        t += '           <div class="bs-options-wrapper">' + _createDays() + "</div>",
        t += "       </div>",
        t += "   </fieldset>",
        t += '   <div class="cddsw-foot-wrapper">',
        t += '       <button class="btn-confirm">确定</button>',
    ...
    fn._createDaySetupInputGroup = function() {
        ...
        r = 0; r < a.length; r++) {
            var n = a[r];
            console.log(n),
            !1 !== n.editable && (t = n.disabled ? "disabled": "", e += "<li>", e += "   <label>" + n.name + "</label>", e += '   <input name="' + n.key + '" placeholder="' + n.placeholder + '" type="' + (n.type || "text") + '" ' + t + ">", e += "</li>")
        }
        return e
    },
    ...
    fn.handleClickEvent = function() {
        ...
        var r = !1;
        function t(t, e) {
            c.settingWindow.find('.cddsw-form-wrapper [type="text"]').val(""),
            c.settingWindow.find('[name="enableDateRange"]').prop("checked", !1),
            c.settingWindow.find('[name="setWeek"]').prop("checked", !1),
            $.each(t,
            function(t, e) {
                c.settingWindow.find('[name="' + t + '"]').val(e)
            }),
            c.settingWindow.find('[name="startDay"], [name="endDay"]').val(e),
            a = c.settingWindow.find(".bs-days-select .bs-options-wrapper"),
            c.settingWindow.show(),
            verticalCenter(c.settingWindow.find(".cddsw-container")),
            r = !1,
            a.removeClass("disabled-options"),
            c.settingWindow.find(".bs-week-chekbox ._active").removeClass("_active")
        }
        function e() {
            r = !0,
            a.addClass("disabled-options")
        }
        function s() {
            var t = [];
        ...
        this.calendar.on("click", ".prev-month",
        function() {
            var t = c.getMonthData();
            c.ev["month-change"] && c.$emit("month-change", t),
            c.opts.monthChange(t),
            c._prevMonth()
        }),
        this.calendar.on("click", ".next-month",
        function() {
            var t = c.getMonthData();
            c.ev["month-change"] && c.$emit("month-change", t),
            c.opts.monthChange(t),
        ...
        this.calendar.on("click", ".btn-reset",
        function() {
            c.ev.reset && c.$emit("reset"),
            c.data = c._getOptionsData(),
            c.createCalendar(),
            c.opts.reset()
        }),
        this.calendar.on("click", ".btn-confirm",
        function() {
            c.ev.confirm && c.$emit("confirm", c.data),
            c.opts.callback(c.data)
        }),
        this.calendar.on("click", ".btn-cancel",
        function() {
            c.ev.cancel && c.$emit("cancel"),
            c.opts.cancel()
        }),
        this.calendar.on("click", ".valid-hook",
        function() {
            if (!c.opts.disableSetup) {
                ...
                c.opts.everyday ? c.opts.everyday(e) : c.ev["valid-day"] ? c.$emit("valid-day", currentDay, e,
                function() {
                    t(e, currentDay)
                }) : t(e, currentDay)
            }
        }),
        this.settingWindow.on("click", ".bs-week-chekbox ._checkbox",
        function() {
            var t = $(this);
        ...
        this.settingWindow.on("click", ".bs-days-select ._checkbox",
        function() {
            var t = $(this);
        ...
        this.settingWindow.on("click", ".cddsw-close, .btn-cancel",
        function() {
            c.ev["setup-close"] && c.$emit("setup-close"),
            c.settingWindow.hide()
        }),
        this.settingWindow.on("click", ".btn-confirm",
        function() {
            var a = {};
            ...
            c.ev.hasOwnProperty("submit-form") ? c.$emit("submit-form", i,
            function() {
                l(i)
            }) : l(i)
        }),
        this.settingWindow.on("change", "input",
        function() {
            var t = $(this),
            e = {
                name: t.attr("name"),
    ...
    fn._getInitDaysRange = function() {
        for (var t, e = [], a = this.startDate.getTime(), r = (this.endDate.getTime() - a) / ONE_DAY_MSEC + 1, n = 0; n < r; n++) t = new Date(a + n * ONE_DAY_MSEC),
        e.push(formatDate(t, "yyyy-MM-dd"));
        return e
    },
    ...
    fn.handleThisData = function(t, e) {
        var a = e || [],
        r = a.length;
        if (0 === r) t.date = currentDay,
        this._updateThisData(t);
    ...
    fn.createStyleCode = function() {
        ...
        count = 0;
        for (var key in style) if (count++, 0 < count) break;
        if (count) {
            var defaultStyle = {
            ...
            reg = null;
            for (var key in concatStyle) reg = eval("/{" + key + "}/g"),
            templateStyle = templateStyle.replace(reg, concatStyle[key]);
            $("head").append("<style>" + templateStyle + "</style>")
        }
    },
    fn.sort = function(t) {
        if (! (t instanceof Array)) return this.ev.error && this.$emit("error", _err(8)),
        this.opts.error(_err(8)),
        t;
        if (t.length < 1) return t;
        for (var e = 0,
        a = null,
        r = 0; r < t.length; r++) for (var n = (e = r) + 1; n < t.length; n++) t[n].date < t[e].date && (e = n, a = t[r], t.splice(r, 1, t[n]), t.splice(n, 1, a));
        return t
    },
    ...
    fn.getMonthData = function() {
        ...
        r = new RegExp("^" + a + "-\\d+"),
        n = [];
        for (t = 0; t < this.data.length; t++) e = this.data[t],
        r.test(e.date) && n.push(e);
        return {
            month: a,
            data: n
        }
    ...
    fn.update = function(t) {
        if (! (t && t instanceof Array)) return this.ev.error && this.$emit("error", _err(9)),
        void this.opts.error(_err(9));
        var e, a, r, n;
        for (r = this.rmRepeat(t, "date"), e = 0; e < r.length; e++)(a = r[e]).stock *= 10,
        null === (n = this._getArrIndex(a, this.data)) ? this.data.push(a) : this.data.splice(n, 1, a);
        this.renderDataToTalbe()
    },
    fn._getArrIndex = function(t, e) {
        for (var a = 0; a < e.length; a++) if (t.date == e[a].date) return a;
        return null
    },
    $.extend({
